...
├─ M2 shipyard 后端骨架                ├─ M6 pipeline 模板管理       ├─ M9 snapshot + 回滚
├─ M3 shipyard Web 骨架                └─ M7 env_variable + 注入     ├─ M10 LLM adapter
...
| **涉及文件** | `shipyard/pom.xml`, `shipyard/src/main/java/.../Application.java`, `shipyard/src/main/resources/application.yml`, `shipyard/src/main/resources/db/migration/V1__init.sql` |
...
| 项 | 内容 |
|---|---|
| **目标** | shipyard 能调 GitLab API 拉仓库、读 commit、创建分支、commit 文件、创建 MR |
| **涉及文件** | `shipyard/src/main/java/com/shipyard/repo/RepoAdapter.java` (interface), `shipyard/src/main/java/com/shipyard/repo/gitlab/GitLabAdapter.java`, `shipyard/src/main/java/com/shipyard/repo/gitee/GiteeAdapter.java` (stub), `shipyard/src/main/java/com/shipyard/repo/RepoFactory.java` |
| **验收** | • interface 定义: `getRepo()`, `getFile()`, `createBranch()`, `commitFile()`, `createMR()`<br>• GitLabAdapter 用 `gitlab4j-api` 实现,所有方法跑通<br>• GiteeAdapter 返回 `UnsupportedOperationException("first issue: 详见 docs/CONTRIBUTING")`<br>• Factory 按 `project.repo_provider` 返回对应 adapter<br>• shipyard 调 GitLab API 真实创建分支 + commit + MR 成功 |
...
| 项 | 内容 |
|---|---|
| **目标** | shipyard 能调 drone 触发构建 + 接收 webhook + HMAC 验签 |
| **涉及文件** | `shipyard/src/main/java/com/shipyard/build/DroneClient.java`, `shipyard/src/main/java/com/shipyard/build/WebhookController.java`, `shipyard/src/main/java/com/shipyard/build/HmacVerifier.java` |
...
| **涉及文件** | `shipyard/src/main/java/com/shipyard/pipeline/`, `shipyard/src/main/java/com/shipyard/ai/`, `web/src/views/PipelineEdit.vue`(完整实现) |
...
| **涉及文件** | `shipyard/src/main/java/com/shipyard/variable/`, `shipyard/src/main/java/com/shipyard/crypto/AesEncrypter.java`, `shipyard/src/main/java/com/shipyard/build/VariableInjector.java` |
| **验收** | • `env_variable` 表 CRUD<br>• AesEncrypter: AES-256/GCM,启动时读 `application.yml` 的 shipyard key<br>• **envelope encryption 接口设计**: `Encrypter` interface,V1 实现 AesEncrypter,以后接 KMS 只换实现<br>• VariableInjector 拼 `vars.yaml` (解密后),不落盘,直接走 stdin 传给 drone<br>• 启动时全量校验变量完整性,启动失败列出损坏变量 |
...
| 项 | 内容 |
|---|---|
| **目标** | worker 部署进 k3s 集群,能接 shipyard 任务做 deploy/rollback/stop |
...
| **测试** | • 单元测试: deploy 状态机<br>• 集成测试: 真实 k3s 集群 apply deployment<br>• **PBT 用 jqwik... 不对,Go 用 gopter** — snapshot 拼装用 gopter(实际拼装在 shipyard,见 M9)<br>• 镜像大小验证: < 20MB |
| **估算** | 2-3 天 |
| **依赖** | M2(API 端点定下来) |
...
| 项 | 内容 |
|---|---|
| **目标** | shipyard 拼完整 deployment snapshot,deploy 用它,回滚用历史 snapshot |
| **涉及文件** | `shipyard/src/main/java/com/shipyard/deploy/SnapshotBuilder.java`, `shipyard/src/main/java/com/shipyard/deploy/DeployService.java` |
...
| **涉及文件** | `shipyard/src/main/java/com/shipyard/ai/LlmAdapter.java` (interface), `MockLlmAdapter.java`, `TongyiLlmAdapter.java`, `DiagnosisService.java`, `DecisionService.java` |
...
| 项 | 内容 |
|---|---|
| **目标** | shipyard 自带 4-5 套 Dockerfile 模板,生成后提交进项目仓库走 MR |
| **涉及文件** | `shipyard/src/main/java/com/shipyard/dockerfile/`, `shipyard/src/main/resources/templates/*.mustache`, `web/src/views/CreateProject.vue`(完整), `web/src/views/DockerfileFlow.vue`(生成流) |
...
| 项 | 内容 |
|---|---|
| **目标** | shipyard 实时转发 drone 日志给前端,构建完落 MySQL |
| **涉及文件** | `shipyard/src/main/java/com/shipyard/build/LogStreamController.java`, `shipyard/src/main/java/com/shipyard/build/LogPersistenceService.java`, `web/src/views/BuildDetail.vue`(完整) |
| **验收** | • SSE endpoint: GET `/api/builds/{id}/logs/stream` 返回 `SseEmitter`<br>• shipyard 用 WebClient 调 drone stream API 转发<br>• 持久化: webhook 推 BuildSuccess/Failed 时,shipyard 同步调 drone logs API 写 `build_log` 表<br>• 完整日志: GET `/api/builds/{id}/logs` 返回已持久化日志<br>• Web BuildDetail 页用 EventSource 接收,失败时弹 AI 诊断按钮 |
...
| **验收** | • CI workflow 跑: 单元测试 + 集成测试 + lint + coverage 报告 + hadolint<br>• 覆盖率门槛: shipyard 后端 ≥ 70%, worker ≥ 60%, 前端 ≥ 50%(PR 合入要求)<br>• spotless + prettier pre-commit hook<br>• Conventional Commits + commitlint<br>• 每个文件 Apache 2.0 header(用 spotless plugin)<br>• Dependabot 自动更新 PR<br>• release-please 自动生成 CHANGELOG |
...
| **验收** | • 一键拉起: shipyard + MySQL + Redis + drone + drone-runner + Harbor + k3s + worker(k3s 内部) + Prometheus + Grafana<br>• 演示脚本: 5 分钟 demo 视频(屏幕录制)<br>• 关键截图 5-6 张<br>• README 完整(中英双语)+ 架构图(4 张 mermaid)+ Quick Start<br>• docs/interview-prep.md(Mavis 帮整理): 5 分钟讲稿 + 20 个 Q&A + 3 个踩坑故事<br>• CONTRIBUTING.md 写清楚 first issue(Gitee 适配器) |
...
| 风险 | 触发条件 | 回退方案 |
|---|---|---|
| **M5 drone 集成卡住** | drone 1.x API 不稳定或文档不全 | 切到 Tekton(用同样的 shipyard 调 API 模式) |
